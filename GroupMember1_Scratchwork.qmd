...
orders6 <- orders5 %>%
  ...
  filter(n() >= 10) %>%     # Filter groups with at least 20 orders
...
write.csv(NYCFoodOrders, "FoodOrdersNYC.csv", row.names = FALSE)
```
...
NYCFoodOrders$total_time <- NYCFoodOrders$prep_time + NYCFoodOrders$delivery_time
```
...
american_restaurants <- NYCFoodOrders[NYCFoodOrders$cuisine_type == "American", ]
...
model_american <- lmer(cost ~ 1 + (prep_time | restaurant), data = american_restaurants)
...
Japanese_restaurants <- NYCFoodOrders[NYCFoodOrders$cuisine_type == "Japanese", ]
```
