...
FoodOrdersNYC$total_time <- FoodOrdersNYC$prep_time + FoodOrdersNYC$delivery_time
```
...
Y_{ij} = \beta_{0} + \beta_{1} \text{cuisineType}_{i} + \beta_{2} \text{prepTime}_{ij} + u_{i} + \epsilon_{ij}

$$


$$
u_i\sim N \left( \text{0} \text{,}\sigma^{2}_{u}\right) \\ \epsilon_i\sim N \left( \text{0} \text{,}\sigma\right)
$$
